...
- Added the source and derived-page foundation for workspace knowledge collections. Versioned collection schemas and access policies now govern immutable inline snapshots or content-addressed source references, stable source revision and supersession chains, classification ceilings, workspace isolation, and matching file/SQLite repositories. Derived Markdown pages preserve stable keys and aliases, typed metadata, bounded version history, review state, claim-level source citations, outgoing links, and atomically recomputed backlinks without duplicating alias-matched pages. Paginated REST endpoints expose collection, source, and cited page metadata under work-product permissions while stored source content remains internal for the upcoming reviewed ingestion and cited-query layers (#867).
