...
- Added the source, derived-page, reviewed-ingestion, cited-search, and query-promotion foundation for workspace knowledge collections. Versioned collection policies govern immutable inline snapshots or content-addressed references, source revision chains, classification ceilings, and workspace isolation. Cited Markdown pages preserve stable keys and aliases, typed metadata, bounded history, review state, outgoing links, and atomically recomputed backlinks without duplicating alias-matched pages. Durable dry runs now show exact page, metadata, link, index, contradiction, and activity changes before mutation. Administrator apply and reverse operations bind the exact proposal and page digests, update the graph, proposal state, and append-only activity in one file replacement or SQLite transaction, retry idempotently, and refuse blocking contradictions, stale pages, or unsafe reversal without partial changes. Collection-scoped search distinguishes raw evidence from derived synthesis, carries claim citations, respects workspace RBAC, and projects current derived pages into isolated, non-default QMD collections with explicit keyword fallback. Search responses now carry a digest, and selected results can create a normal dry-run ingestion proposal whose persisted provenance binds the query, evidence digest, and selected result IDs. Paginated REST endpoints expose the reviewed workflow under work-product permissions while redacted export and launch restrictions remain follow-up slices (#867).
