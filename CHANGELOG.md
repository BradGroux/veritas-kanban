...
- Added the source-catalog foundation for workspace knowledge collections. Versioned collection schemas and access policies now govern immutable inline snapshots or content-addressed source references, stable source identity, revision and supersession chains, classification ceilings, workspace isolation, retry-safe operation identities, and matching file/SQLite repositories. The initial REST API exposes collection and source metadata under work-product permissions while stored source content remains internal for the upcoming reviewed ingestion and cited-query layers (#867).
