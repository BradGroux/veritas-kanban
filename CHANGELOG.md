...
- Added the source, derived-page, reviewed-ingestion, cited-search, query-promotion, and cited-export foundation for workspace knowledge collections. Versioned collection policies govern immutable inline snapshots or content-addressed references, source revision chains, classification ceilings, workspace isolation, and export posture. Cited Markdown pages preserve stable keys and aliases, typed metadata, bounded history, review state, outgoing links, and atomically recomputed backlinks without duplicating alias-matched pages. Durable dry runs now show exact page, metadata, link, index, contradiction, and activity changes before mutation. Administrator apply and reverse operations bind the exact proposal and page digests, update the graph, proposal state, and append-only activity in one file replacement or SQLite transaction, retry idempotently, and refuse blocking contradictions, stale pages, or unsafe reversal without partial changes. Collection-scoped search distinguishes raw evidence from derived synthesis, carries claim citations, respects workspace RBAC, and projects current derived pages into isolated, non-default QMD collections with explicit keyword fallback. Search responses carry a digest; selected results can create a normal dry-run ingestion proposal or a Markdown work product whose body, source links, metadata, and later Markdown/JSON exports preserve citations. Forbidden collections cannot export, and redacted-only products reject full export overrides. Paginated REST endpoints expose the reviewed workflow under work-product permissions while launch-manifest restrictions remain a follow-up slice (#867).
...
- Added workspace team roster manifests and capability routing rules for
  capability-aware delegation (#734).
- Added cross-workspace capability discovery and delegated work intake (#735).
- Added policy-gated queue intake monitoring for GitHub issues and agent runs
  (#736).
- Added a unified recurring work scheduler for monitors, workflows, and digests
  (#737).
- Added Squad Chat threads, unread state, mentions, and search (#738).
- Added bidirectional human reply adapters for Squad Chat and notifications
  (#739).
- Added ceremony enforcement for design reviews and failure retrospectives
  (#740).
- Added reflection-to-memory promotion for agent corrections and lessons (#741).
- Added external tracker schema introspection for configurable work items
  (#742).

### Changed

- Bumped the release from `5.1.0` to `5.2.0` across the root, shared, server,
  web, CLI, MCP, and desktop package manifests.
- Updated the README version badge, API reference, v5 release notes, and MCP
  documentation footer for `5.2.0`.

### Fixed

- Fixed CLI/MCP compatibility smoke checks so missing `VK_API_KEY` records
  explicit skip results after local package/build validation instead of failing
  unrelated release gates (#753).
- Removed vulnerable frontmatter dependencies from the server by using the
  existing YAML parser and preserving malformed-prompt fallback behavior
  (#754).
- Upgraded DOMPurify and kept the Mantine QA gate below the initial bundle
  budget by deferring drag/drop code outside the first page load (#754, #755).
