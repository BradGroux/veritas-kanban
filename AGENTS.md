...
- Run `pnpm validate:release -- --version X.Y.Z --native-evidence /absolute/path/evidence.json --native-app /absolute/path/veritas-kanban.app --media-evidence /absolute/path/capture/publication.json` against the clean publication checkout. The publication manifest must bind the untouched capture from the exact packaged build; only the documented regular documentation/media paths may change afterward. Source or dependency changes require a fresh build and capture. The post-publication `--github` form also requires the published GitHub body and tag to match the reviewed publication commit. `--source-only` is a separate source preflight, not release acceptance; `--skip-build-output` does not bypass native or documentation-media evidence.
