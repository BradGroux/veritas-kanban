...
- **Provider-neutral progress watchdog** — A versioned, bounded evaluator detects identical tool, error, and assistant-tail repetition, short multi-step cycles, repeated failed edits, and sustained time or spend without durable progress from the shared run journal. Policy controls confidence escalation, progress signals, allowed repetition leases, and recovery posture. A restart-safe coordinator journals attributed findings and action outcomes, rehydrates per-turn and per-run recovery use, and invokes only a registered bounded executor; otherwise the decision stays operator-required
