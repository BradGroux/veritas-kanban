...
- **Provider-neutral progress watchdog evaluator** — A versioned, bounded evaluator detects identical repetition, short multi-step cycles, repeated failed edits, and sustained activity without durable progress from the shared run journal. Findings retain only redacted event references and hashes, honor explicit time- and rate-bounded repetition leases, and select recovery posture under per-turn and per-run budgets; active journal integration and enforcement remain a separate delivery slice
