...
- **Dependency health and load shedding** — Provider/model calls, MCP discovery and tools, outbound integrations, and storage operations report into durable restart-safe `dependency-circuit/v1` state. Failure-rate and slow-call thresholds open circuits; bounded half-open probes test recovery; route selection excludes unhealthy candidates; shared retry budgets prevent retry amplification; deep health exposes redacted evidence; and admin-only reset plus expiring allow/block overrides provide governed recovery controls
