...
The rewind coordinator requests a critical, non-mobile approval whose exact action binds the stable preview evidence, runtime state, provider evidence revision, checkpoints, and estimated loss. It leaves the provider running while approval is pending. Once approved, a provider runtime port must quiesce the exact runtime state, after which the coordinator regenerates the preview and requires the same stable evidence before starting storage mutation. Runtime cursor commit occurs only after the storage transaction commits. If runtime commit fails, the coordinator rolls the committed storage transaction back before allowing runtime rollback; a failed storage rollback deliberately leaves the provider quiesced for recovery.
...
This foundation does not yet claim exact overlapping-hunk attribution, selective conflict resolution, a production provider-runtime port, an operator-facing route, or shared blob garbage collection. Those layers must consume the immutable repository and remain preview-first. Production wiring must implement provider quiescence plus atomic conversation-cursor and run-metadata commit/rollback behind the coordinator's runtime contract.
