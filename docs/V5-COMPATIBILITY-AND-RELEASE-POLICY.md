...
| SQLite schema        | A v5 app may open supported v5 schema versions only. File-backed v4 data upgrades through the migration service.                                | `PRAGMA user_version`, active `schema_migrations`, migration journal, recovery state.        | Older apps refuse newer SQLite databases with `SQLITE_UNSUPPORTED_SCHEMA` and direct the admin to a compatible app or pre-migration backup restore. | Dual-storage parity tests, migration fixture tests, `docs/MIGRATION-RECOVERY.md`.                                |
...
[#545](https://github.com/BradGroux/veritas-kanban/issues/545). v5 clients must
keep run controls, evidence views, and approval boundaries server-owned and
client-mode aware.

Linux and Windows desktop artifacts generated by `Desktop Artifacts` are
preview-only for v5 GA. They keep post-GA packaging paths exercised, but they
...
Linux and Windows preview artifacts cannot be promoted to `stable` or linked as
supported release downloads under this v5 GA policy.
