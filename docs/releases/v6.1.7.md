...
Create Task, template application, previews, conflict resolution, and other task confirmations retain their context while requests are pending or fail. A partial blueprint cannot be blindly retried after some tasks were created. Nested popouts restore focus to visible controls, and quickly closing and reopening a task no longer lets stale cleanup remove the new workspace. Reduced-motion mode no longer introduces unintended layout transitions during window or text-size changes.
