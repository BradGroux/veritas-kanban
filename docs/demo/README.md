...
2. Run `pnpm docs:capture-media /absolute/path/veritas-kanban.app /absolute/path/new-capture-directory`. The directory must not exist and must be outside the checkout. The command launches an isolated authenticated packaged server, captures desktop and 390×844 mobile views, and writes the media, raw interaction frames, and original capture evidence there. It never replaces the installed app or copies media into the repository automatically.
3. Inspect every still and play both GIFs and `demo-overview.mp4`. Reject clipped controls, incomplete interactions, private content, or stale UI. A completed capture is not release acceptance.
...
5. From the clean documentation-only commit, run `pnpm docs:verify-media /absolute/path/new-capture-directory/evidence.json /absolute/path/veritas-kanban.app /absolute/path/new-capture-directory/publication.json`. It verifies the original capture, package identity, documentation-only Git delta, and exact committed bytes without rebuilding or recapturing. Follow the [media contract](../design/DOCUMENTATION-MEDIA-CONTRACT.md); app changes require a fresh build and capture. Never relabel historical preparation evidence.
