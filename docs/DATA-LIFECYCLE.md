...
| Work products and versions                          | `work_products`, `work_product_versions`, `work_product_artifacts`, `task_deliverables`                                                                                                                                                     | Active and archived metadata plus immutable file bodies until explicit physical cleanup | Full/scoped exports preserve encoded bodies; support bundles exclude bodies; preview source, versions, bytes, integrity, and quarantine state before deletion. |
...
SQLite export manifests use `formatVersion: 3` and include:
...
Format 3 adds `work_product_artifacts`. Binary content is represented with an explicit base64 wrapper in JSON and decoded back to a SQLite BLOB during import. File-storage backups preserve `.veritas-kanban/work-product-artifacts/` recursively. Support bundles never include either backend's file bodies.
