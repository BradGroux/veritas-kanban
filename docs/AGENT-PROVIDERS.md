...
- `codex-sdk`, `codex-app-server`, `claude-code`, `acp-stdio`, and `hermes` have
  executable adapters but are disabled by default.
...
`2.1.218 (Claude Code)` version match and probe revision 10; version or build
...
`provider-runtime-manifest/v1` evidence at probe revision 10. Launch starts a
...
Credential-bound server definitions remain blocked until brokered launch
handles land in #932.
