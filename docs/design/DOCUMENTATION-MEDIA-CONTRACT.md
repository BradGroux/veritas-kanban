...
The runner also records ten `task-<drawer|expanded>-<overview|plan|run|results|history>.png` comparisons in the same versioned directory. These remain separate from the maintained inventory in `taskModeAssets`. If any is published, the complete ten-image capture manifest is required. Each has the same candidate and file-hash checks, an actual 1180×900 native window, and measured content dimensions. The manifest records fixture and framing differences: native content capture excludes the frame and shadow retained in the original 6.1.7 task-mode audit. Do not describe unmatched data or host conditions as identical.
...
Permitted post-build changes are regular, non-executable README/documentation Markdown, `docs/index.json`, `docs/index.html`, the demo player at `docs/demo/index.html`, the 14 versioned maintained media files, the ten explicitly named task-mode comparison PNGs with their complete capture evidence, the optional verified `board-5000.png`, and the two demo MP4 paths. Source, dependency, build, version, executable, or other changes require a new clean build and fresh captures. A documentation-only commit cannot justify rewriting an older manifest, changing recorded bytes, or reusing a different app package. Historical preparation/recapture reports remain diagnostic and cannot be promoted to publication evidence by editing their mode or identity.

### Focused large-board comparison

The optional `focusedBoardAssets` capture list permits only `board-5000.png`, at 1360×900 native content pixels. It has the same build, app digest, timestamp, byte-hash and committed-file requirements as the maintained captures. A published focused image requires its original capture entry.

Run `node --import tsx ../scripts/native-ui/board-performance.mjs <candidate.app> <new-evidence-directory>` from `server/`. After that feature run passes, supply its `report.json` as the final argument to `pnpm docs:capture-media <candidate.app> <new-capture-directory> capture <board-report.json>`. The capture command verifies the same candidate identity and original screenshot hash before incorporating the focused image. This supplements the fourteen maintained assets and ten task-mode views; it does not replace them.
