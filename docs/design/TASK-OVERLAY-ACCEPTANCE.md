...
| Work product versions, editing, artifact preview  | `WorkProductsSection`, `ArtifactPreviewModal` | Light/dark; three sizes; both motion settings; text/image/HTML controls; PDF rendering unresolved (#1448)                 | Pending                 |
...
Light minimum-size HTML and dark enlarged-window image captures were inspected. These are browser diagnostics, not final documentation media. Four PDF cases are explicitly `fixme` under #1448: a valid one-page document produced a blank frame despite passing footer and zoom-label checks. PDF page rendering is not accepted. The required passive sandbox remains unchanged; #1448 must prove visible pages and safe rendering in browser and packaged macOS before release acceptance.
