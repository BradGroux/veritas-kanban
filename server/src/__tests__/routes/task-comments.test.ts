...
describe('Task Comments Routes', () => {
  ...
  describe('POST /api/tasks/:id/comments', () => {
    ...
    it('should reject missing fields', async () => {
      ...
      const res = await request(app).post(`/api/tasks/${task.id}/comments`).send({});
  ...
  describe('DELETE /api/tasks/:id/comments/:commentId', () => {
    it('should delete a comment', async () => {
      ...
      // Delete the comment
      const res = await request(app).delete(`/api/tasks/${task.id}/comments/${commentId}`);
    ...
    it('should delete only the specified comment', async () => {
      ...
      const res = await request(app).delete(`/api/tasks/${task.id}/comments/${secondCommentId}`);
    ...
    it('should return 404 for non-existent task', async () => {
      const res = await request(app).delete('/api/tasks/nonexistent_id/comments/comment_123');
    ...
    it('should return 404 for non-existent comment', async () => {
      ...
      const res = await request(app).delete(`/api/tasks/${task.id}/comments/nonexistent_comment`);
